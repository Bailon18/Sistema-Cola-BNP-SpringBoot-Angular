...
export class GenerarticketComponent implements OnInit {
  ...
  guardarTicket(servicioId: number, servicioNombre: string, moduloId: number, clienteId: number): void {
    ...
    this.servicio.guardarTicket(ticket).subscribe(
      ...
      (resp) => {
        ...
        // this.atencionesService.guardarAtenciones(atencion).subscribe({
        //   next:(resp) =>{
        //     console.log("Guardado correctamente "+resp)
        //   },
        //   error:(error) =>{
        ...
        this.generarContenidoTicket(numeroTicket, nombreModulo, fecha, nombreservicio);
  ...
  generarContenidoTicket(numeroTicket: string, nombreModulo: string, fecha: any, nombreservicio: string): void {
    ...
    if (fecha instanceof Date) {
      pdf.text("Fecha: " + fecha.toLocaleDateString(), 10, 50);
    } else {
    ...
    iframe.onload = () => {
      ...
      setTimeout(() => {
        ...
        //   html: 'El ticket se ha creado con éxito.'
        // });
